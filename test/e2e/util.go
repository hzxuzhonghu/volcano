...
func updateJob(context *context, job *batchv1alpha1.Job) error {
	spec, err := json.Marshal(job.Spec)
	if err != nil {
		return err
	}
	patchBytes := []byte(fmt.Sprintf(`{"spec":%s}`, spec))
	_, err = context.vcclient.BatchV1alpha1().Jobs(job.Namespace).Patch(job.Name, types.StrategicMergePatchType, patchBytes)
	return err
}
